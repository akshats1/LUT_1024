...
motor.move_xyz_u(x=X_START, y=Y_START)   # position over slide
time.sleep(0.3)                            # settle
motor.move_xyz_u(z=Z_START)               # then approach depth
...
def find_initial_z():
    """
    Coarse Z search — move THEN capture at every step.
    Never captures at the arrival position before searching.
    Order: move → settle → capture → evaluate.
    """

    step  = -0.045   # 45 micron steps downward
    steps = 15       # 0.675mm total range

    logger.info(f"[Initial Z] Coarse search: {steps} steps x {step}mm  start Z={motor.z:.4f}")
    ...
    for i in range(steps):
        ...
        time.sleep(0.15)          # let stage settle after move

        # ── 2. CAPTURE + EVALUATE after move ────────────────────────
        ...
        if var > max_var:
            max_var = var
            best_z  = motor.z   # snapshot motor.z — never overwrite it manually

    # Move back to best Z found — motor.z updated internally by move_xyz_u
    motor.move_xyz_u(z=best_z - motor.z)
    time.sleep(0.2)
    logger.info(f"[Initial Z] Best Z={motor.z:.4f} | Var={max_var:.2f}")

    return motor.z   # return ground truth, not stale snapshot
...
try:
    ...
    for row in range(ROWS):
        ...
        for step_i, col in enumerate(col_indices):
            ...
            filename = f"cervical_{serp_index:05d}.tiff"
            filepath = os.path.join(save_dir, filename)

            img_path = camera.capture_fullres_image_to(filepath)

            logger.info(
                f"[SCAN] row={row:02d} col={col:02d} "
                f"idx={serp_index:04d} Z={motor.z:.5f} → {filename}"
            )
